...
function DespesasPage() {
  ...
  // Filter only expense transactions for the selected month
  const filtered = useMemo(() => {
    const prefix = `${currentYear}-${String(currentMonth + 1).padStart(2, "0")}-`;
    return transactions
      .filter(t => t.type === "despesa" && t.date.startsWith(prefix))
      .sort((a, b) => b.date.localeCompare(a.date));
  }, [transactions, currentMonth, currentYear]);

  const totals = useMemo(() => {
    const total = filtered.reduce((sum, t) => sum + Math.abs(t.amount), 0);
    const paid = filtered.filter(t => t.paid).reduce((sum, t) => sum + Math.abs(t.amount), 0);
    ...
    <div className="mx-auto w-full max-w-7xl space-y-5 px-3 py-4 sm:px-4 sm:py-6 md:px-8 md:py-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <Badge className="bg-destructive text-destructive-foreground px-4 py-1.5 text-sm font-semibold rounded-full cursor-default select-none">
            ↓ {t("common.expense") || "Despesas"}
          </Badge>
        </div>
        <div className="flex items-center gap-2">
          <Button asChild className="gap-1.5 bg-destructive text-destructive-foreground hover:bg-destructive/90 shadow-sm">
            <Link to="/registros">
              <Plus className="h-4 w-4" />
              {t("expense.new") || "Nova Despesa"}
            </Link>
          </Button>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }}>
          <Card className="border-border/60 shadow-[var(--shadow-card)] cursor-pointer hover:border-destructive/40 transition-colors">
            <CardContent className="flex items-center gap-4 p-4">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-destructive/15">
                <ArrowUpCircle className="h-5 w-5 text-destructive" />
              </div>
              <div className="min-w-0 flex-1">
                <p className="text-xs text-muted-foreground">{t("expense.pending") || "Despesas pendentes"}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
          <Card className="border-border/60 shadow-[var(--shadow-card)] cursor-pointer hover:border-destructive/40 transition-colors">
            <CardContent className="flex items-center gap-4 p-4">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-xs text-muted-foreground">{t("expense.paid") || "Despesas pagas"}</p>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border/40">
                ...
                <TableHead>{t("rec.col.date") || "Data"}</TableHead>
                <TableHead>{t("rec.col.description") || "Descrição"}</TableHead>
                <TableHead className="flex items-center gap-1">
                  {t("rec.col.category") || "Categoria"}
                </TableHead>
